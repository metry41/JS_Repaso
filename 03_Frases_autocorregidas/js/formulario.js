class Formulario {
	...
	// Creamos el "modelo" corespondiente a todos los elemetos existentes en el DOM
		...
		// Incorporación de los textos al DOM
	...
	controller () {
		...
		this.oDOM.eFrase.addEventListener("input", (oEvent) => this.actualizarVista(oEvent));
		this.oDOM.eBtnBorrar.addEventListener("click",(oEvent) => this.actualizarVista(oEvent))
		for (var i = 0; i < this.oDOM.eSamples.children.length; i++) {
			this.oDOM.eSamples.children[i].addEventListener("click",(oEvent) => this.actualizarVista(oEvent));			
		}
	}
	 ...
	 * @return number
	 * 
	 * 0 => Mezcla de Mayúsculas y Minúsculas
	 * 1 => Todo minúsculas
	 * 2 => Todo mayusculas 
	 * 
	 */
